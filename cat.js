...
if (options.includes("-n") && options.includes("-b")){//both are present
...
for(let i=0;i<arr.length;i++){
    if(arr[i]===''|| arr[i]=="\r"){            
        if(flag===true){
            continue;       // loop ki agli iteration chalu, else k bd skip
        }else{
            y.push(arr[i])
            flag=true
        }
...
//-n Adding Line number to all empty elements in arr
...
//-b Adding Line number to only non empty elements
